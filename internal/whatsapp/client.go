...
// SendDocument sends a document file to WhatsApp
func (c *Client) SendDocument(ctx context.Context, to string, fileData []byte, filename string, mimetype string, caption string) error {
	jid, err := types.ParseJID(to)
	if err != nil {
		jid = types.NewJID(to, types.DefaultUserServer)
	}
	...
	uploaded, err := c.WAClient.Upload(ctx, fileData, whatsmeow.MediaDocument)
	if err != nil {
